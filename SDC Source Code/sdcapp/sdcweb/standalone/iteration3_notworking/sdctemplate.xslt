...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="form-action" select="document('sr-service.xml')/sr:sr-service"/>
	...
	<xsl:template name="box">
		<textarea style="width:90%;margin:40px;background-color:lightyellow;" rows="40" id = 'rawxml'> 
			<!--<xsl:copy-of select="."/>-->
	...
	<xsl:template match="*" mode="doe">	
	   ...
	   <!--
		...
		<xsl:copy>			
			<xsl:apply-templates select="node()|@*" mode="doe"/>
		</xsl:copy>-->
		<!--
		<xsl:choose>
